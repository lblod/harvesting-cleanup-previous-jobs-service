...
async function removeFile(file) {
  ...
  try {
    ...
    if (e.code === 'ENOENT') {
      console.warn(`Physical file not found (may already be deleted): ${file.fileOnDisk}`);
      physicalFileDeleted = true; // Consider it deleted if it doesn't exist
    } else {
      console.error(`Failed to delete physical file ${file.fileOnDisk}: ${e.message}`);
    }
  }
  ...
  try {
    ...
    dbFileError = e;
    console.error(`Failed to delete file in database: ${file.file}. ${e.message}`);
  }

  // Report overall status
  if (!physicalFileDeleted || !dbFileDeleted) {
    const errors = [];
    if (!physicalFileDeleted) errors.push(`physical file: ${physicalFileError?.message}`);
    if (!dbFileDeleted) errors.push(`database: ${dbFileError?.message}`);
    throw new Error(`Failed to fully delete file ${file.file}. Errors: ${errors.join(', ')}`);
  }
}
