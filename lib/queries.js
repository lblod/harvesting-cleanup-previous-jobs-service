...
export async function getLastDumpFileJobDate() {
  const queryStr = `
    ...
    SELECT  ?modified  WHERE {
      graph <${DEFAULT_GRAPH}> {
        ${operationValues(DUMP_FILE_OPERATIONS)}
...
`;
  ...
  if (response?.results?.bindings?.length) {
    ...
    console.log("no last dump file found");
    return new Date(0); // we keep everything (Since 1970 ;-P)
  }
}

function operationValues(operations) {
  if (operations.length === 0) {
    return '';
  } else {
    return `VALUES ?operation {${operations
...
export async function getSuccessfulJobsBeforeDate(date) {
  ...
  const selectAllJobsAndCollectingContainer = `
    ...
    SELECT DISTINCT ?job ?jobId ?modified ?dataContainer WHERE {
      ${operationValues(OPERATIONS)}
      GRAPH <${DEFAULT_GRAPH}> {
        ?job a ?type;
             adms:status <http://redpencil.data.gift/id/concept/JobStatus/success>;
             mu:uuid ?jobId;
             dct:modified ?modified.
        ?tasks dct:isPartOf ?job;
               tasks:operation ?operation;
               adms:status <http://redpencil.data.gift/id/concept/JobStatus/success>.
        ?tasks tasks:resultsContainer ?dataContainer.
        FILTER (?type IN (cogs:Job, cogs:ScheduledJob))
      }
    }
  `;
  ...
  for (let [rootUrl, jobs] of jobsMap.entries()) {
    ...
    while (jobs.length) {
      ...
      if (j.modified < maxDaysToKeepSuccessFulJobs) {
        jobsToClean.push({jobUri: j.jobUri, jobId: j.jobId});
      } else {
        console.log(
