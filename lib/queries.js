...
import { DEFAULT_GRAPH, MAX_DAYS_TO_KEEP_SUCCESSFUL_JOBS } from "../constants";
  ...
  const q = `select distinct ?job where {
    graph <${DEFAULT_GRAPH}> {
      ?job a <http://vocab.deri.ie/cogs#Job>;
           ...
           <http://purl.org/dc/terms/modified> ?modified.
      filter (?modified < ${sparqlEscapeDateTime(date)})
...
`;
  ...
  for (const job of response?.results?.bindings) {
    ...
    const dataContainer = job.job.dataContainer.value;
    const queryRootUrl = `
...
`;
    ...
    if (res.results?.bindings?.length === 1) {
      let rootUrl = cleanupUrl(res.results.bindings[0].targetUrl.value);
